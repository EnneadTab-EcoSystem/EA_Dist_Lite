...
def merge_layer():
    ...
    # keep reopening the modal after each merge so the user can merge repeatedly.
    # Cancel in the modal is the only way to exit.
    while True:
        all_layers = rs.LayerNames()
        all_layers_user = [RHINO_LAYER.rhino_layer_to_user_layer(x) for x in all_layers]
        active_layer_user = RHINO_LAYER.rhino_layer_to_user_layer(rs.CurrentLayer())
        """
        print(all_layers[10])
        ...
        layers_to_modify, layer_target = RHINO_FORMS.select_from_list2list(all_layers_user,
                                                                            ...
                                                                            width = 600,
                                                                            height = 500,
                                                                            disabled_items_A = [active_layer_user],
                                                                            disabled_reason_A = "This is the active layer. Rhino cannot merge the active layer into another layer.\nSet a different layer active first.")


        # Cancel returns (None, None); any other empty selection means Merge was
        # clicked with nothing usable, so re-open the modal instead of exiting.
        if layers_to_modify is None and layer_target is None:
            return
