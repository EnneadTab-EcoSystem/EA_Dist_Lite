...
1. PROJECT DETAILS (get_acc_projects_data):
   ...
   - Saved as: ACC_PROJECTS_DETAILS.sexyDuck
...
2. PROJECT SUMMARY (get_ACC_summary_data):
   ...
   - Saved as: ACC_PROJECTS_SUMMARY.sexyDuck
...
3. PROJECT BY YEAR (generated alongside summary):
   ...
   - Saved as: ACC_PROJECTS_BY_YEAR.sexyDuck
...
# ============================================================================
# Main data file names - these are the production files that stay up to date
# Note: These will automatically get .sexyDuck extension from FOLDER module
ACC_PROJECTS_DETAILS = "ACC_PROJECTS_DETAILS"
ACC_PROJECTS_SUMMARY = "ACC_PROJECTS_SUMMARY" 
ACC_PROJECTS_BY_YEAR = "ACC_PROJECTS_BY_YEAR"
...
def get_acc_projects_data(use_record = True):
    ...
    File Location: ACC_PROJECTS_DETAILS.sexyDuck (shared dump folder)
    Regeneration: Every 21 days or when manually cleared
    ...
    """
    save_file = ACC_PROJECTS_DETAILS
    if use_record:
...
def clear_acc_cache():
    """Clear all ACC-related data files to force fresh API calls.
    
    This function removes all ACC data files including:
    - Project details
    - Project summaries  
    ...
    Returns:
        bool: True if files were cleared successfully
    """
    try:
        cache_files = [
            ACC_PROJECTS_DETAILS,
            ACC_PROJECTS_SUMMARY,
            ACC_PROJECTS_BY_YEAR
        ]
        ...
        # Also clear individual project Revit files (these have dynamic names)
        dump_folder = FOLDER.get_shared_dump_folder()
        if os.path.exists(dump_folder):
            for filename in os.listdir(dump_folder):
                if filename.startswith("ACC_PROJECT_REVIT_FILES_"):
                    try:
                        file_path = os.path.join(dump_folder, filename)
                        os.remove(file_path)
                        cleared_count += 1
                        logging.info("Cleared file: {}".format(filename))
                    except Exception as e:
                        logging.warning("Failed to clear file {}: {}".format(filename, e))
...
def get_ACC_summary_data(show_progress = False):
    ...
    WHAT THIS GENERATES:
    1. PROJECT SUMMARY (ACC_PROJECTS_SUMMARY.sexyDuck):
    ...
    2. PROJECT BY YEAR (ACC_PROJECTS_BY_YEAR.sexyDuck): 
    ...
    REGENERATION TRIGGERS:
    - File age > 21 days (CACHE_EXPIRY_DAYS)
    - Data files missing or corrupted
    - Debug mode enabled (forces fresh data)
    - Manual file clearing
    
    File Locations: 
    - ACC_PROJECTS_SUMMARY.sexyDuck (shared dump folder)
    - ACC_PROJECTS_BY_YEAR.sexyDuck (shared dump folder)
...
class ACC_PROJECT_RUNNER:
    ...
    def run_an_idle_job(self, year_version = None, debug = False):
        ...
        # Check if summary data needs regeneration (missing, debug mode, or older than 3 weeks)
