...
def repath_to_root(path):
    """Re-root an absolute path embedded in a .sexyDuck spec to the current ROOT.

    The specs are auto-generated by auto-py-to-exe and embed the ABSOLUTE path of
    whatever checkout the author built from (e.g.
    C:\\Users\\szhang\\github\\EnneadTab-OS\\... or the ennead-llp org variant).
    Re-root everything from the "EnneadTab-OS" repo-root marker onward at ROOT so
    builds work from any worktree, machine, or org path. A path with no marker is
    returned unchanged. Shared by ExeBuilder._repath and the GUI mtime sorter so
    ...
    if idx == -1:
        return path
    tail = path[idx + len(marker):]  # e.g. "\\DarkSide\\exes\\source code\\X.py"
    return ROOT + tail
...
class ExeBuilder:
    ...
    def _repath(self, path: str) -> str:
        ...
        Returns:
            str: Converted path
        """
        # Delegate to the shared module-level helper so this and the GUI mtime
        # sorter (get_source_mtime) re-root paths identically and cannot drift.
        return repath_to_root(path)
                    ...
                    with open(os.path.join(EXE_MAKER_FOLDER, sexyduck_file), 'r') as f:
                        ...
                        for opt in data.get('pyinstallerOptions', []):
                            if opt.get('optionDest') == 'filenames':
                                ...
                                if source_path:
                                    # Same generic re-rooting as ExeBuilder._repath
                                    # (was a narrow one-prefix replace that only
                                    # handled the ennead-llp org path).
                                    source_path = repath_to_root(source_path)
