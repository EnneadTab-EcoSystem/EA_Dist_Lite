"""
EnneadTab for Rhino -- modern guided wizard (shared by installer + uninstaller).

Light 3-screen flow: welcome -> progress -> done. Self-contained chrome/theme,
copied verbatim from _revit_wizard.py so the two evolve independently (see
that file's own note re: the OS wizard). All work runs on a background
thread; the Rhino attach/detach itself lives in enneadtab_for_rhino_core.
...
from enneadtab_for_rhino_core import EnneadTabRhinoInstallationManager

# Dark theme -- same muted charcoal + teal palette as the Revit wizard.
BG = "#1A1D21"
CARD = "#24282E"
INK = "#E8EAED"
MUTED = "#9AA3AD"
...
class RhinoWizard(tk.Tk):
    ...
    def __init__(self, is_installing: bool = True):
        ...
        self.title("EnneadTab for Rhino {}".format("Installer" if is_installing else "UnInstaller"))
        ...
        self.manager = EnneadTabRhinoInstallationManager(
    ...
    def _build_chrome(self):
        ...
        tk.Label(
            header,
            text="{} EnneadTab for Rhino".format(self.verb),
    ...
    def _step_welcome(self, preview_only=False):
        self._heading(self.body, "{} EnneadTab for Rhino".format(self.verb))
        if self.is_installing:
            self._para(self.body, "This will attach EnneadTab to Rhino so the EnneadTab "
                                  "toolbar appears the next time you open Rhino -- no need to "
                                  "drag a .rui file by hand. It will:")
            self._bullet(self.body, "Briefly launch Rhino in the background to load the toolbar")
            self._bullet(self.body, "Register EnneadTab to load automatically every session")
        else:
            self._para(self.body, "This will detach EnneadTab from Rhino. It will:")
            self._bullet(self.body, "Briefly launch Rhino in the background to close the toolbar")
            self._bullet(self.body, "Remove EnneadTab's automatic-load registration")
        self._para(self.body, " ")
        self._para(self.body, "Close Rhino before continuing so the change takes effect cleanly.")

        status = tk.Label(self.body, text="", font=("Segoe UI", 10, "bold"), bg=BG, anchor="w")
        status.pack(anchor="w", pady=(10, 4))
        recheck_btn = ttk.Button(self.body, text="Re-check Rhino")
        recheck_btn.pack(anchor="w", pady=(2, 0))

        # The polling worker below fires its UI update via self.after(0, ...)
        # from a background thread. If the user navigates away in the
        # meantime (e.g. clicks Install before the 20s poll resolves),
        # _clear_body() destroys `status`/`recheck_btn` and that queued
        # callback then hits a dead Tcl widget -- confirmed live 2026-09-08
        # ("invalid command name ...!label5"). Every access below is guarded
        # with winfo_exists(), same defensive pattern as _append_log's
        # widget-may-be-gone guard elsewhere in this file.
        def set_status(running):
            if not status.winfo_exists():
                return
            if running:
                status.config(text="\u25cf  Rhino is currently running \u2014 please close it.", fg=WARN)
            else:
                status.config(text="\u25cf  Rhino is not running.", fg=OK)

        def quick_check():
            set_status(self.manager.check_rhino_running())

        def poll_after_close():
            # Rhino's process can linger up to ~20s after its window closes --
            # EnneadTab's own background threads (the MCP server, InfraWatch
            # registration) outlive the closed main window, confirmed live
            # 2026-09-08. A one-shot check right after closing reports "still
            # running" and stays stuck there until the user manually clicks
            # again; poll instead so it resolves on its own.
            recheck_btn.config(state="disabled")
            status.config(text="\u25cf  Checking... (Rhino can take up to 20s to fully close)", fg=MUTED)

            def worker():
                still_running = True
                deadline = time.time() + 20
                while time.time() < deadline:
                    if not self.manager.check_rhino_running():
                        still_running = False
                        break
                    time.sleep(1.5)
                self.after(0, lambda: finish(still_running))

            def finish(still_running):
                set_status(still_running)
                if recheck_btn.winfo_exists():
                    recheck_btn.config(state="normal")

            threading.Thread(target=worker, daemon=True).start()

        recheck_btn.config(command=poll_after_close)
        if not preview_only:
            quick_check()
        else:
            status.config(text="\u25cf  Rhino is currently running \u2014 please close it.", fg=WARN)
    ...
    def _step_progress(self, preview_only=False):
        ...
        self._para(self.body, "Rhino will briefly open in the background -- this is expected. "
        ...
        if preview_only:
            for line in ("Looking for EnneadTab OS...",
                         "Found Rhino 8: C:\\Program Files\\Rhino 8\\System\\Rhino.exe",
                         "Launching Rhino to attach the EnneadTab toolbar (no drag-and-drop needed)..."):
    ...
    def _step_done(self, preview_only=False):
        ...
        if ok:
            ...
            if self.is_installing:
                self._para(self.body, "EnneadTab is attached to Rhino. Open Rhino and look for "
                                      "the EnneadTab toolbar.", fg=OK)
            else:
                self._para(self.body, "EnneadTab has been detached from Rhino. You can close "
                                      "this window.", fg=OK)
        else:
...
def run(is_installing: bool) -> int:
    app = RhinoWizard(is_installing=is_installing)
    app.mainloop()
    return 0
